            ...
            div { class: "space-y-6",
                div { class: "bg-card rounded-2xl p-5 border border-border shadow-sm",
                    div { class: "flex items-start gap-4",
                        ...
                        div { class: "flex-1",
                            h4 { class: "font-bold text-lg mb-1 text-foreground", "Beta Program" }
                            p { class: "text-[11px] text-muted-foreground font-medium mb-4 leading-relaxed", "Get early access to new features and improvements before they are released." }
                            
                            if beta_joined() {
                                button { 
                                    class: "w-full h-12 bg-accent/50 hover:bg-accent border border-border rounded-xl text-xs font-bold transition-all active:scale-95 text-foreground flex items-center justify-center",
                                    onclick: move |_| beta_joined.set(false),
                                    "Leave Beta" 
                                }
                            } else {
                                button { 
                                    class: "w-full h-12 bg-primary hover:brightness-110 text-primary-foreground rounded-xl text-xs font-bold transition-all active:scale-95 shadow-lg shadow-primary/20 flex items-center justify-center",
                ...
                div {
                    h4 { class: "text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-3 ml-1", "What's New" }
                    div { class: "space-y-4",
                        ChangeLogItem { 
                            version: "v0.1.5", 
                            date: "Jan 29, 2026", 
